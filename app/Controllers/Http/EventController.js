...
class EventController {
  ...
  async store ({ request, response, auth }) {
    ...
    try {
      ...
      return response.status(401).send({
        error: {
          message: 'Não é possivel definir dois eventos no mesmo horário.'
        }
      })
    } catch (err) {
  ...
  async show ({ params, request, response, auth }) {
    ...
    if (event.user_id !== auth.user.id) {
      return response
        .status(401)
        .send({ error: { message: 'Apenas o criador do evento pode ver.' } })
    }
  ...
  async destroy ({ params, response, auth }) {
    ...
    if (event.user_id !== auth.user.id) {
      return response.status(401).send({
        error: { message: 'Apenas o criador do event opode excluir.' }
      })
    }
    ...
    if (passed) {
      return response
        .status(401)
        .send({ error: { message: 'Você não pode excluir eventos passados.' } })
    }
